...
export async function GET() {
  ...
  try {
    ...
    if (!yearsRes.ok || yearsData.errors) {
      console.error(
        "GitHub GraphQL error (years):",
        yearsData.errors || yearsRes.statusText,
      );
      return new NextResponse("Failed to fetch contribution years", {
        status: 500,
      });
    }
    ...
    if (!contributionsRes.ok || contributionsData.errors) {
      console.error(
        "GitHub GraphQL error (contributions):",
        contributionsData.errors || contributionsRes.statusText,
      );
      return new NextResponse("Failed to fetch contribution data", {
        status: 500,
      });
    }
    ...
    const sortedDailyContributions = dailyContributions.sort((a, b) =>
      b.date.localeCompare(a.date),
    );
    const maxDailyCount = Math.max(
      ...sortedDailyContributions.map((entry) => entry.count),
      1,
    );
      ...
      contributions: sortedDailyContributions.map((entry) => {
        const intensity =
          entry.count === 0
            ...
            : Math.min(
                4,
                Math.max(1, Math.ceil((entry.count / maxDailyCount) * 4)),
              );
    ...
    if (error instanceof Error) {
      return new NextResponse(`Failed to fetch: ${error.message}`, {
        status: 500,
      });
    }
